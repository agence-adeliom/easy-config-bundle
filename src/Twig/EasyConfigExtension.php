    ...
    public function getConfig(Environment $env, array $context, string $key, bool $directValue = true)
    {
        if ($config = $this->configRepository->getByKey($key)) {
            if ($directValue) {
                ...
                if (in_array($config->getType(), [EasyConfigEnum::CODE->value, EasyConfigEnum::WYSIWYG->value, EasyConfigEnum::TEXTAREA->value, EasyConfigEnum::TEXT->value])) {
                    return new Markup($value, 'UTF-8');
                } elseif (EasyConfigEnum::JSON->value == $config->getType()) {
